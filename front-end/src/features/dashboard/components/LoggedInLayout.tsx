...
export function LoggedInLayout({
  ...
  // Helper function to load and set health metrics from API response
  const loadHealthMetricsFromResponse = (res: any) => {
    if (res.success && res.data && res.data.length > 0) {
      // Get the latest record (sorted by recordedAt or RecordedAt)
      ...
      if (goal && typeof goal === 'object') {
        ...
        // Extract ExercisePerWeek from goal.plan.ExercisePerWeek
        ...
        // Fallback: check if recommendedSessions is directly in goal
        if (!recommendedSessions) {
          const sessions = (goal as any).recommendedSessions || (goal as any).RecommendedSessions || (goal as any).recommendedDays || (goal as any).RecommendedDays
  ...
  // Load health metrics immediately when component mounts (for sidebar display)
  useEffect(() => {
    let cancelled = false
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Silently fail - user can still navigate to health page to add data
        }
      }
    })()
  ...
  // Load saved health metrics from API when component mounts or when entering health section
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (metrics) {
          ...
          setIsEditingHealth(false) // Start in view mode
        } else {
          // No records found, start in edit mode
          setIsEditingHealth(true)
        }
      } catch (error) {
        console.error('Failed to load health metrics:', error)
        // If error, start in edit mode
        setIsEditingHealth(true)
      } finally {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // Check if user has health metrics before loading training data
        if (!healthMetrics || !healthMetrics.bmi) {
          ...
          // Load health metrics from API response
          const metrics = loadHealthMetricsFromResponse(bmiRes)
          if (!metrics || !metrics.bmi) {
            // Still no valid metrics, redirect to health page
            setHealthNotice('Vui lòng nhập chỉ số sức khoẻ trước khi sử dụng tính năng Tập luyện.')
            onSelectSection('health')
            setIsEditingHealth(true)
            return
    ...
    if (section === 'training') {
      ...
      if (!healthMetrics || !healthMetrics.bmi) {
        ...
        try {
          const res = await getMyBmiRecords()
          if (res.success && res.data && res.data.length > 0) {
            // User has health metrics, load them and allow access
            const metrics = loadHealthMetricsFromResponse(res)
            if (metrics && metrics.bmi) {
        ...
        setHealthNotice('Vui lòng nhập chỉ số sức khoẻ trước khi sử dụng tính năng Tập luyện.')
        onSelectSection('health')
        setIsEditingHealth(true) // Open in edit mode
        return
      }
    }
  ...
  const handleHealthSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Reload BMI records from database to get the latest data including Goal
      try {
        ...
        if (bmiRecordsRes.success && bmiRecordsRes.data && bmiRecordsRes.data.length > 0) {
          ...
          // Fallback to response data if no records found
      ...
      if (activeSection === 'training') {
        try {
          // Reload BMI records to ensure training page has latest data
            ...
            const getSectionIcon = () => {
              switch (section) {
                case 'training':
                  return (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M20.57 14.86L22 13.43L20.57 12L17 15.57L8.43 7L12 3.43L10.57 2L9.14 3.43L7.71 2L5.57 4.14L4.14 2.71L2.71 4.14L4.14 5.57L2 7.71L3.43 9.14L7 5.57L15.57 14L12 17.57L13.43 19L14.86 17.57L16.29 19L18.43 16.86L19.86 18.29L21.29 16.86L19.86 15.43L20.57 14.86Z" fill="currentColor"/>
                    </svg>
                  )
                case 'nutrition':
                  return (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M8.1 13.34L11 16.24L18.07 9.17L20.59 11.69C21.37 12.47 21.37 13.72 20.59 14.5L13.93 21.16C13.15 21.94 11.9 21.94 11.12 21.16L8.1 18.14C7.32 17.36 7.32 16.11 8.1 15.33L8.1 13.34ZM4.83 2.69L8.1 5.96L6.68 7.38L3.41 4.11C2.63 3.33 2.63 2.08 3.41 1.3C4.19 0.52 5.44 0.52 6.22 1.3L9.49 4.57L10.9 3.16L7.63 -0.11C6.85 -0.89 5.6 -0.89 4.82 -0.11C4.04 0.67 4.04 1.92 4.82 2.7L4.83 2.69Z" fill="currentColor"/>
                    </svg>
                  )
                case 'progress':
                  return (
                ...
                case 'social':
                  return (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M16 4C18.21 4 20 5.79 20 8C20 10.21 18.21 12 16 12C13.79 12 12 10.21 12 8C12 5.79 13.79 4 16 4ZM16 14C18.67 14 24 15.33 24 18V20H8V18C8 15.33 13.33 14 16 14ZM8 10C10.21 10 12 8.21 12 6C12 3.79 10.21 2 8 2C5.79 2 4 3.79 4 6C4 8.21 5.79 10 8 10ZM8 12C5.33 12 0 13.33 0 16V18H8.5C7.56 17.16 7 16.13 7 15C7 13.83 7.47 12.73 8.22 11.88C8.15 11.95 8.08 12 8 12Z" fill="currentColor"/>
                    </svg>
                  )
                default:
                  return null
                ...
                <button type="submit" className="btn-primary" disabled={healthSaving}>
                  ...
                  <button 
                    ...
                    onClick={() => {
                      ...
                      setHealthError(null)
                      setHealthSuccess(null)
                      // Reset form to saved values if they exist
                      if (healthMetrics) {
                        setHealthForm({
                      ...
                      {(sessionItems[order] || []).map((item, idx) => (
                        <div className="workout-row" key={`session-${order}-row-${idx}`}>
                          ...
                          <div className="row-actions">
                            ...
                            >
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M6 19C6 20.1 6.9 21 8 21H16C17.1 21 18 20.1 18 19V9C18 7.9 17.1 7 16 7H8C6.9 7 6 7.9 6 9V19ZM19 4H15.5L14.5 3H9.5L8.5 4H5V6H19V4Z" fill="currentColor"/>
                              </svg>
                            </button>
                          </div>
                        </div>
                        ...
                        exercises.map((ex) => (
                          ...
                          >
                            <div>
                              <p className="exercise-title">{ex.title}</p>
                              <p className="exercise-meta">
                                {(ex.primaryMuscle || '---')} · {(ex.caloriesBurnedPerSet || (ex as any).CaloriesBurnedPerSet || 0)} cal/set
                              </p>
                            </div>
                            <button
                              type="button"
              ...
              {(() => {
                ...
                return embedUrl ? (
                  ...
                  <div className="exercise-detail-section">
                    ...
                    <a href={exerciseDetailModal.videoUrl} target="_blank" rel="noopener noreferrer" className="btn-primary">
